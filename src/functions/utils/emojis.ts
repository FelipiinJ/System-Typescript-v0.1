...
export function icon(name: EmojiKey) {
    ...
    const id = animated ? settings.emojis.animated[name.slice(3, name.length) as keyof EmojiList["animated"]] : settings.emojis.static[name.slice(3, name.length) as keyof EmojiList["static"]];
